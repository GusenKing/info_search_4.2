    ...
    for file_path in folder_path.glob("*_леммы.txt"):
        ...
        with open(file_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                # idf для термина одинаковый во всех документах поэтому можго взять один раз
                if lemma not in idf:
                    idf[lemma] = idf_value
...
def build_query_vector(query: str, idf: dict[str, float]): 
    #считаем tf-idf для запроса, 
    # чтобы потом сравнить его с документами
...
def compute_cosine_similarity( doc_vector: dict[str, float], query_vector: dict[str, float]):
    ...
    norm_query = math.sqrt(sum(v * v for v in query_vector.values())) # евклидова длина вектора
...
def vector_search_multi(queries: list[str], tf_idf: dict[str, dict[str, float]], idf: dict[str, float]):
    for query in queries:
        
        # print(f"\nЗапрос: {query}")
        # print("Леммы запроса:", lemmatize_query(query))
        # print("Ключи idf (первые 20):", list(idf.keys())[:20])
